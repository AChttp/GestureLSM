...
class GestureMF(torch.nn.Module):
    ...
    # @Kael: add u_fn
    def u_fn(self, x_t, t, r, seed_vectors, audio_features_flow):
        model_output = self.denoiser(
            x = x_t,
            timesteps = t.reshape(x_t.shape[0]),
            cond_time = r.reshape(x_t.shape[0]), #!注意 这里应该是r 还是 t-r？？？
            seed = seed_vectors,
    ...
    def train_forward(self, condition_dict: Dict[str, Dict], 
                              latents: torch.Tensor) -> Dict[str, torch.Tensor]:
        # Disable fused attention for JVP computation
        self._disable_fused_attn_for_jvp()
        # Extract input features
        audio = condition_dict['y']['audio_onset']
        word_tokens = condition_dict['y']['word']
        ...
        t, r = self.sample_tr(latents.shape[0], latents.device)
        
        '''
            Follow this work: 
                https://github.com/HaoyiZhu/MeanFlow-PyTorch/blob/cf7cd11aa46bcee571aba6d6dc9e868a90325edd/train_meanflow.py
                https://github.com/HaoyiZhu/MeanFlow-PyTorch/blob/cf7cd11aa46bcee571aba6d6dc9e868a90325edd/meanflow.py
            loss function:
                loss, proj_loss, v_loss
        '''
        ...
        u, du_dt = self.jvp_fn(u_fn, (x_t, t, r), (v_guided, dtdt, dtdr))
